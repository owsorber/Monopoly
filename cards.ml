...
let draw_chance_card t = 
  if t.chance_int < (Array.length t.chance_deck - 1) 
    then (let c = Array.get t.chance_deck t.chance_int in t.chance_int <- 
    t.chance_int + 1; c) 
...
let draw_community_chest_card t = 
  if t.community_chest_int < (Array.length t.community_chest_deck - 1) 
    then (let c = Array.get t.community_chest_deck t.community_chest_int in 
    t.community_chest_int <- t.community_chest_int + 1; c;)
...
let rec find_nearest board loc typ =
  ...
  | Railroad r -> if typ = "rr" then loc else find_nearest board (loc + 1) typ
  | Utility u -> if typ = "utility" then loc else find_nearest board (loc + 1) 
    typ
  | _ -> find_nearest board (loc + 1) typ

let move_nearest player board extra game = 
  move_card (find_nearest board (Player.get_location player) extra) player

let rec num_houses game list house hotel = 
  match list with
  | [] -> (house, hotel)
  |h :: t -> let num = Game.get_houses game h in if num = 5 then num_houses game
    t (house + 4) (hotel +1) else num_houses game t (house + num) (hotel)
...
let property_charges game player extra = 
  ...
  Player.update_balance player (-(fst num * (List.hd list) + snd num * 
  (List.nth list 1)))

let add_others_funds player game amount = 
  Array.iter (fun p -> Player.pay p player amount) (Game.get_all_players game)

let do_card card p board game=
  match card.action with
  |"move" -> move_card (Board.location_from_space_name board card.extra) p 
  |"addfunds" -> change_funds p card.extra
  |"quarantine"-> quarantine_card p card.extra
  |"removefunds" -> change_funds p card.extra
  |"movenum" -> move_card ((Player.get_location p) + 
    int_of_string card.extra) p 
  |"movenearest"-> move_nearest p board card.extra game
  |"propertycharges" -> property_charges game p card.extra
  |"removefundstoplayers" -> add_others_funds p game (int_of_string card.extra)
  |"addfundsfromplayers" -> add_others_funds p game (int_of_string card.extra)
  |_ -> raise NotValidCard
